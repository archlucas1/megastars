...
def CURVE_fit(x, y):
    ...
    y_high = f(x, a+a_err, b+b_err)
    y_low = f(x, a-a_err, b-b_err)

    # plot it
    plt.plot(x, a*x+b, label='best curve_fit: y = ('+result(a,a_err)+')x + ('+result(b,b_err)+')', color = 'black')
    plt.fill_between(x, y_high, y_low, alpha=0.3, color = 'black')
...
# bootstrapping 
def bootstrap_fit(x,y,x_error,y_error,n_samples=5000):
...
def bootstrap_fit_2(f, x, y, x_err, y_err, conf_pct=95, n_samples=5000):
...
#Bootstrapping
bootstrap_fit_2(f, x_array, y_array, x_error_array, y_error_array) #using ODR fit as above for each sample
bootstrap_fit(x_array, y_array, x_error_array, y_error_array) #independant method to the one above, using curve_fit
